...
export function validateModels() {
  ...
  let limit = 1; // pogo.platforms.length;

  return new Promise((resolve, reject) => {
    const validate = (index) => {
      let platform = 'android'; // pogo.platforms[index].name;
      ...
      if (!this.fileExists(path + "asset_digest")) {
        ...
        shared.GAME_ASSETS[platform] = POGOProtos.Networking.Responses.GetAssetDigestResponse.decode(buffer);
      }

      let files = getFileInfoFromFolder(path);

      // validate models inside folder
      let ii = 0;
      while (++ii <= max) {
        let id = idToPkmnBundleName(ii);
        let asset = getAssetByBundleName(id, shared.GAME_ASSETS[platform].digest);
        ...
        if (correspondence) {
          fs.copyFile(path + correspondence.name, path + id, (e) => {
